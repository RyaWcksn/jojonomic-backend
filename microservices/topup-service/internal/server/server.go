...
import (
	...
	"github.com/RyaWcksn/jojonomic-backend/topup-service/internal/app/handler"
	"github.com/RyaWcksn/jojonomic-backend/topup-service/internal/app/service"
	"github.com/RyaWcksn/jojonomic-backend/topup-service/internal/architecture/message"
	...
	"github.com/RyaWcksn/jojonomic-backend/topup-service/internal/server/middleware"
...
func SRV(cfg config.Config, log logger.ILogger) {
	...
	serviceImpl := service.NewService(brokerImpl, log, priceImpl)
	handler := handler.NewHandler(serviceImpl, log)

	r := mux.NewRouter()
	r.Handle("/api/topup", middleware.ErrHandler(handler.TopupHandler)).Methods(http.MethodPost)
	srv := &http.Server{
		Handler:      r,
